...
WORKDIR /src
COPY ["LambdaProcessor.csproj", "LambdaProcessor/"]
RUN dotnet restore "LambdaProcessor/LambdaProcessor.csproj"

WORKDIR "/src/LambdaProcessor"
COPY . .
RUN dotnet build "LambdaProcessor.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "LambdaProcessor.csproj" \
...
Execute unit tests
```
cd "LambdaProcessor/test/LambdaProcessor.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "LambdaProcessor/src/LambdaProcessor"
